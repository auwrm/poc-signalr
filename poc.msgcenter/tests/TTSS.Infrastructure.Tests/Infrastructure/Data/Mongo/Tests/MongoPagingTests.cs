...
{
    public class MongoPagingTests : XUnitTestBase, IDisposable
    {
        private readonly MongoDbRunner dbRunner;
        private readonly MongoRepository<SimpleMongoDocument, string> sut;

        public MongoPagingTests(ITestOutputHelper testOutput) : base(testOutput)
        {
            ...
            var store = new MongoConnectionStoreBuilder(Guid.NewGuid().ToString(), dbRunner.ConnectionString)
                ...
                .Build();
            sut = new MongoRepository<SimpleMongoDocument, string>(store, it => it.Id);
        }
            ...
            int expectedPageCount, int expectedDataElements)
        {
            var records = Enumerable.Range(1, contents)
                .Select(it => new SimpleMongoDocument { Id = it.ToString(), Name = it.ToString() });
